...
def collect_camera_perception_series(
    ...
    *,
    ...
    RCL deliberately does not own camera capture or perception runtimes. The
    deployment injects both. Every frame must have a retained evidence reference,
    and every inference must expose a semantic predicate with confidence.
    """
...
def build_opencv_frame_reader(
    ...
    *,
    ...
    """Build an optional OpenCV camera reader suitable for Jetson deployments.

    OpenCV is imported lazily so the RCL core and CI remain dependency-neutral.
    `source` may be a V4L2 index or a GStreamer pipeline string supported by the
    deployment OpenCV build. Captured JPEG files are retained as evidence refs.
    """
