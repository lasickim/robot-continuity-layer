...
from .expression_optimization_cli import run as run_expression_optimization
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "revise-intent":
        try:
            ...
            print(f"ERROR: {exc}")
            return 2
    if len(sys.argv) >= 2 and sys.argv[1] == "expression-recommendations":
        try:
            return run_expression_recommendation(sys.argv[2:])
        except (RCLValidationError, ValueError, OSError) as exc:
            print(f"ERROR: {exc}")
            return 2
